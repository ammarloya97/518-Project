...
xtraining = xtraining - means;
%pcarotation
Cov = (1/P)*xtraining*xtraining';
[V,D] = eig(Cov);
wtraining = real(V)'*xtraining;
%divide off std

sigma = sqrt(1/P * sum((wtraining).^2,2));
wtraining = wtraining./sigma;
...
%meancentre
xtesting = xtesting - means;
%pca rotation
wtesting = real(V)'*xtesting;
%divide off std
wtesting = wtesting./sigma;

%%
%classifier
w0 = rand(size(x,1)+1,1);
f = @(w)cross_entropy(w,wtraining,ytraining);

% train classifier
[W,fW] = gradient_descent(f,w0,0.1,1e-5,100000);

%%
%organising results
xbar = [ones(1,length(ytesting));wtesting];
W= W(:,end);
